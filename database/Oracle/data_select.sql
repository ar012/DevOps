...
RIGHT OUTER JOIN:
================
SSELECT sales.sales_date, sales.order_id, sales.product_id, product.product_name
FROM sales RIGHT OUTER JOIN product
ON sales.product_id = product.product_id;

Old Syntax of RIGHT OUTER JOIN:
==============================
SELECT sales.sales_date, sales.order_id, sales.product_id, product.product_name
FROM sales , product
WHERE sales.product_id(+) = product.product_id;
...
N.B. "NATURAL JOIN" is equivalent to "INNER JOIN"

JOIN 3 Tables
=============
SELECT sales.sales_date, sales.order_id, sales.product_id, product.product_name, salesperson.job_title
FROM sales INNER JOIN product
ON sales.product_id = product.product_id
...
LEFT OUTER JOIN salesperson
ON sales.salesperson_id = salesperson.salesperson_id
AND job_title = 'East Manager'
INNER JOIN customer
ON sales.customer_id=customer.customer_id;

Add a filter after joins are completed:
=======================================
SELECT sales.sales_date, sales.order_id, sales.product_id, product.product_name, salesperson.job_title, 
customer.first_name, customer.last_name
FROM sales INNER JOIN product
